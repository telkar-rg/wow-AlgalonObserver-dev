...
function AO:OnStarHealthUpdate() 	-- called every second to subtract 1% HP
	...
	for id, bar in pairs(self.stars) do
...
function AO:COMBAT_LOG_EVENT_UNFILTERED(eventName, tstmp, combatevent, srcId, srcName, srcFlags, dstID, dstName, dstFlags, spellID, spellName, spellSchool, spellDmg)
	...
	elseif combatevent == "SPELL_DAMAGE" then
		if spellID == BHE_ID and (not self.stars_exploded[srcId]) then 	-- if spell=BlackHoleExplosion and srcGuid is not yet marked as exploded
	...
	if dstName == L["Collapsing Star"] then
		if combatevent == "SWING_DAMAGE" then
			-- dmg = spellID 	-- "SWING_DAMAGE": arg9 = dmg amount (stored in "spellID" in this case)
			self:SetStarHealth(dstID, -spellID) 	-- update hp
		elseif combatevent == "RANGE_DAMAGE" or combatevent == "SPELL_DAMAGE" or combatevent == "SPELL_PERIODIC_DAMAGE" then
			self:SetStarHealth(dstID, -spellDmg) 	-- update hp
		end
	end
end
...
function AO:SetStarHealth(guid, health, takeMin)
	if self.stars_exploded[guid] then return end 	-- ignore stars that are already known to be dead
	
	local bar = self.stars[guid] 	-- fetch star bar
	...
	local h = bar:GetValue()
	if health < 0 then 	-- relative value (CLEU damage decrease star Health)
		bar:SetValue(h + health)
	else 	-- absolute value
		if takeMin then 	-- if we want to choose the minimum health (getting health info via addon msg)
...
function AO:UPDATE_MOUSEOVER_UNIT()
	...
	if (guid:sub(9, 12) == "80BB") then 	-- "Kollabierender Stern" guid (0xF1300080BB------)
		local health = UnitHealth(unitID)
		self:SetStarHealth(guid, health) 	-- update hp
		self:SendCommMessage(NAME, self:Serialize("s1", guid, health), "RAID", nil, "NORMAL") 	-- send info to raid
		
	elseif (guid:sub(9, 12) == "8067") then 	-- "Algalon der Beobachter" (0xF130008067------)
		if not UnitCanAttack("player", unitID) then 	-- detect fight ending
...
function AO:OnComm(prefix, message, distribution, sender)
	...
	if id and health then
		self:SetStarHealth(id, health, true) 	-- (take minimum hp due to comm potentially giving delayed info)
	end
end
